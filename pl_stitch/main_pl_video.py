...
from loader import VideoFolderMask_puzzle, Temporal_RandStep_video_dataset, RepeatDataset
...
def train_pl(args):
    ...
    # 1. Image Dataset (Puzzle - Direct Video Loading)
    pred_size = args.patch_size * 8 if 'swin' in args.arch else args.patch_size
    dataset_image = VideoFolderMask_puzzle(
        root=args.data_path, 
        transform=transform,
    ...
    print(f"Data loaded: there are {len(dataset_image)} frames for puzzle branch.")
    
    # 2. Temporal Dataset (Video - Direct Video Loading)
    dataset_temporal = Temporal_RandStep_video_dataset(
        video_path=args.data_path, 
        img_size=224,
        seq_len=args.frame_number
    )
